...
function gotRemoteStream(e) {
  if (remoteVideo.srcObject !== e.streams[0]) {
    ...
    listofStreams.push(e.streams[0]);
    startRecording(listofStreams);
...
function hangup() {
  ...
  listofStreams = [];
  stopRecording();
  listofStreams.push(localStream);

}

var recorder;

function startRecording(objStream){
 
    recorder = new RecordRTC(objStream, {
      type: 'video',
      mimeType: 'video/webm',
...
function stopRecording(){
  if (recorder){
    ...
    alert("Please start the call again.");
  }
